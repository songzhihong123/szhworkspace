...
public class Kiki {
    ...
    最近CC直播平台在举行中秋之星主播歌唱比赛，假设一开始该女主播的初始人气值为start，能够晋升下一轮人气需要刚好达到end，土豪给主播增加人气的可以采取的方法有：
        a.       点赞  花费x  C币，人气  +2
        b.       送礼  花费y  C币，人气  *2
        c.       私聊  花费z  C币，人气  -2
    ...
    public static int kiki(int add , int times , int del , int start , int end){
    ...
    // aim 表示目标值
    // cur 表示当前的人气值
    // limitAim 人气达到什么程度不需要再尝试了
    // limitCoin 已经使用了的币大到什么程度不需要再尝试了
    // 返回最小币数
    public static int process(int preMoney , int aim ,
        ...
        //人气 -2
        int p2 = process(preMoney + add , aim , add , times , del , cur - 2 , limitAim , limitCoin);
        if(p2 != Integer.MAX_VALUE){
            min = Math.min(min , p2);
        }

        //人气 *2
        int p3 = process(preMoney + add , aim , add , times , del , cur * 2 , limitAim , limitCoin);
